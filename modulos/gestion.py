...
from utils.db import _run_gestion_select
...
def run():
    ...
    vendedores = (
        df_vendedores.sort_values("SlpCode")["SlpName"].dropna().unique().tolist()
    )
    ...
    vendedor_filtro = st.sidebar.selectbox(
        "Filtrar por Vendedor:", options=["Todos"] + vendedores
    )
    cliente_filtro = st.sidebar.selectbox(
        "Filtrar por Cliente:", options=["Todos"] + clientes_filtrados
    )
    ocrcode3_filtro = st.sidebar.selectbox(
        "Filtrar por Línea de Negocio (OcrCode3):",
        options=["Todas"] + ocrcode3_disponibles,
    )
    ...
    with tab1:
        ...
        try:
            ...
            df_ventas = obtener_ventas_mes(
                DB_PATH, anio_mes_anterior, int(mes_anterior)
            )
            ...
            if vendedor_filtro != "Todos":
                slpcode_sel = nombres_vendedores[
                    nombres_vendedores["SlpName"] == vendedor_filtro
                ]["SlpCode"].values[0]
            ...
            resumen_vendedor = (
                df_forecast.groupby("SlpCode")["Monto"].sum().reset_index()
            )
            ...
            tabla_vendedor = pd.merge(
                resumen_vendedor, ventas_vendedor, on="SlpCode", how="outer"
            ).fillna(0)
            tabla_vendedor = tabla_vendedor[
                (tabla_vendedor["Venta Mes Actual"] != 0)
                | (tabla_vendedor["Venta Mes Anterior"] != 0)
            ]
            tabla_vendedor = tabla_vendedor.merge(
                nombres_vendedores, on="SlpCode", how="left"
            )
            tabla_vendedor = tabla_vendedor.rename(columns={"SlpName": "Vendedor"})
            tabla_vendedor["Diferencia"] = (
                tabla_vendedor["Venta Mes Actual"]
                - tabla_vendedor["Venta Mes Anterior"]
            )

            for col in ["Venta Mes Actual", "Venta Mes Anterior", "Diferencia"]:
                tabla_vendedor[col] = tabla_vendedor[col].apply(
                    lambda x: f"{round(x / 1000, 1):,.1f}".replace(",", "X")
                    .replace(".", ",")
                    .replace("X", ".")
                )

            tabla_vendedor = tabla_vendedor[
                ["Vendedor", "Venta Mes Actual", "Venta Mes Anterior", "Diferencia"]
            ]
            ...
            tabla_linea = pd.merge(
                resumen_linea, ventas_linea, on="OcrCode3", how="outer"
            ).fillna(0)
            tabla_linea = tabla_linea[
                (tabla_linea["Venta Mes Actual"] != 0)
                | (tabla_linea["Venta Mes Anterior"] != 0)
            ]
            tabla_linea["Diferencia"] = (
                tabla_linea["Venta Mes Actual"] - tabla_linea["Venta Mes Anterior"]
            )

            for col in ["Venta Mes Actual", "Venta Mes Anterior", "Diferencia"]:
                tabla_linea[col] = tabla_linea[col].apply(
                    lambda x: f"{round(x / 1000, 1):,.1f}".replace(",", "X")
                    .replace(".", ",")
                    .replace("X", ".")
                )
            ...
            tabla_linea = tabla_linea[
                ["OcrCode3", "Venta Mes Actual", "Venta Mes Anterior", "Diferencia"]
            ]
    ...
    with tab2:
        ...
        resumen_cliente = (
            df_forecast.groupby(["CardCode", "ItemCode"])["Monto"].sum().reset_index()
        )
        resumen_cliente.columns = ["CardCode", "ItemCode", "Venta Mes Actual"]

        ventas_cliente = (
            df_ventas.groupby(["CardCode", "ItemCode"])["Total"].sum().reset_index()
        )
        ventas_cliente.columns = ["CardCode", "ItemCode", "Venta Mes Anterior"]

        tabla_cliente = pd.merge(
            resumen_cliente, ventas_cliente, on=["CardCode", "ItemCode"], how="outer"
        ).fillna(0)
        tabla_cliente["Diferencia"] = (
            tabla_cliente["Venta Mes Actual"] - tabla_cliente["Venta Mes Anterior"]
        )

        try:
            nombres_clientes = _run_gestion_select(
                "SELECT CardCode, CardName FROM OCRD"
            )
            nombres_productos = _run_gestion_select(
                "SELECT ItemCode, ItemName FROM OITM"
            )

            tabla_cliente = tabla_cliente.merge(
                nombres_clientes, on="CardCode", how="left"
            )
            tabla_cliente = tabla_cliente.merge(
                nombres_productos, on="ItemCode", how="left"
            )
        except Exception as e:
        ...
        for cliente in sorted(clientes):
            ...
            nombre_cliente = (
                df_cliente["CardName"].iloc[0]
                if pd.notnull(df_cliente["CardName"].iloc[0])
                else cliente
            )
            ...
            def formato_miles(valor):
                return (
                    f"{round(valor / 1000, 1):,.1f}".replace(",", "X")
                    .replace(".", ",")
                    .replace("X", ".")
                )
            ...
            diferencia_display = (
                f"❗ {total_diferencia_str}"
                if total_diferencia < 0
                else total_diferencia_str
            )
            ...
            with st.expander(titulo):
                ...
                df_view["Diferencia"] = df_view["Diferencia"].apply(
