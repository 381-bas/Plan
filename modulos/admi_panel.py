...
def run():
    ...
    with tabs[0]:
        ...
        forecast_items = _run_admin_select("SELECT DISTINCT ItemCode FROM Forecast_Detalle")
        oitm_items = _run_admin_select("SELECT * FROM OITM")

        codigos_oitm = oitm_items["ItemCode"].unique()
        codigos_faltantes = forecast_items[~forecast_items["ItemCode"].isin(codigos_oitm)]

        if codigos_faltantes.empty:
            st.success("✅ No hay productos faltantes en OITM.")
        else:
            st.warning(f"⚠️ Se detectaron {len(codigos_faltantes)} productos sin registrar en OITM.")
            ...
            faltantes_detalle = forecast_detalle[forecast_detalle["ItemCode"].isin(codigos_faltantes["ItemCode"])]
            ...
            uom_options = oitm_items["InvntryUom"].dropna().unique().tolist() if "InvntryUom" in oitm_items else []
            grp_cod_options = sorted(oitm_items["ItmsGrpCod"].dropna().unique().tolist()) if "ItmsGrpCod" in oitm_items else []
            grp_name_options = sorted(oitm_items["ItmsGrpNam"].dropna().unique().tolist()) if "ItmsGrpNam" in oitm_items else []
