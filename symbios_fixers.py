...
- AUTO:
  ...
  * YAML_SAFE:      yaml.load(â€¦) -> yaml.safe_load(â€¦)
...
AUTO_RULES = [
    ...
    (
        "SESSION_READS_ATTR",
        re.compile(r"st\.session_state\.slpcode(?!\s*=\s*)"),
        "get_slpcode()",
    ),
    # Lecturas estilo Ã­ndice ["slpcode"] (evita asignaciones)
    (
        "SESSION_READS_INDEX",
        re.compile(r"st\.session_state\[\s*['\"]slpcode['\"]\s*\](?!\s*=\s*)"),
        "get_slpcode()",
    ),
    ("BARE_EXCEPT", re.compile(r"\bexcept\s*:\s*(#.*)?$"), "except Exception as e:"),
    ("YAML_SAFE", re.compile(r"\byaml\.load\s*\("), "yaml.safe_load("),
]
...
IMPORT_RX = re.compile(
    r"^\s*from\s+session_utils\s+import\s+get_slpcode\b", re.MULTILINE
)
...
def process_file(p: Path) -> dict:
    ...
    # Si tocamos lecturas de sesiÃ³n, asegurar import
    if replaced_any_session:
        new_text = _ensure_import(text)
        if new_text != text:
            changes.append(
                ("INSERT_IMPORT", "from session_utils import get_slpcode", "added")
            )
            text = new_text
...
def main() -> int:
    py_files = [
        p for p in ROOT.rglob("*.py") if p.is_file() and p.name != "symbios_fixers.py"
    ]
    ...
    print(
        f"[SYMBIOS][AUTO] cambios {'aplicados' if APPLY else 'propuestos'}: {total_applied}"
    )
    for r in results:
