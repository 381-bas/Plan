...
- Fechas: formatos sospechosos (%Y-%d-%m), to_datetime ambiguo (multilínea)
- Sesión: claves 'SlpCode' vs 'slpcode' (inconsistencias)
- Excepciones: 'except:' pelado
- Seguridad: eval/exec, pickle, yaml.load inseguro
- Paths: rutas absolutas (C:\, /home/…), IO duro
...
RULES: List[Rule] = [
    ...
    ("DATE_FMT_DMY_STRPTIME", "MED", r"strptime\s*\([^,]+,\s*[\"']%d[-/ ]%m[-/ ]%Y[\"']"),
    # Pandas to_datetime (detector base por línea; se afina en escaneo multilínea)
    ...
    ("IO_HARDCODE", "MED", r"\b(open|pd\.read_(csv|excel|parquet)|to_csv|to_excel)\s*\([^)]*[\"'](C:\\|/home/|/Users/)[^\"']+[\"']"),
    ...
    ("MUTABLE_DEFAULT", "HIGH", r"def\s+\w+\s*\([^)]*(=\s*\{\}|=\s*\[\]|=\s*dict\(\)|=\s*list\(\))"),
]

# --- helpers para inspección multilínea de to_datetime ---
def _gather_call(lines: List[str], start_idx: int) -> str:
    """Devuelve el texto completo de la llamada que inicia en lines[start_idx] hasta balancear paréntesis."""
            ...
            if rid == "DATE_PANDAS_AMBIG":
                if "pd.to_datetime" in line:
                    ...
                    if _to_datetime_is_ambiguous(call):
                        findings.append({"rule": rid, "sev": sev, "file": str(path), "line": i, "code": line.strip()})
                continue
...
def main() -> int:
    ...
    def block(sev: str, title: str):
        ...
        out_md.append(f"## {title}\n")
        for it in items:
            out_md.append(f"- **{it['rule']}** · `{it['file']}:{it['line']}`")
            out_md.append(f"  ```py\n  {it['code']}\n  ```")
        out_md.append("")

    for sev, title in [("CRITICAL","CRITICAL"),("HIGH","HIGH"),("MED","MEDIUM"),("LOW","LOW")]:
        block(sev, title)
